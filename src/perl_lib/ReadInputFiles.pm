...
# code_line is ready to be processed
      if (nonempty($code_blocks[$#code_blocks]{"code_line"})) {
# this will become area that parses solver code
        print UNWRAPPED_INPUT $indent x $#code_blocks,$code_blocks[$#code_blocks]{"code_line"},"\n";
        $code_blocks[$#code_blocks]{"code_line"} = '';

# buffer is ready to be processed
      } elsif ( ( nonempty($code_blocks[$#code_blocks]{"buffer"}) && $code_blocks[$#code_blocks]{"buffer"} !~ /&\s*$/ ) || # buffer is ready to be processed to code_line as it is full and doesn't end with a continuation symbol
                ( empty($code_blocks[$#code_blocks]{"buffer"}) && nonempty($code_blocks[$#code_blocks]{"comments"}) ) ) { # or buffer is empty but comments are present
        $code_blocks[$#code_blocks]{"code_line"} = $code_blocks[$#code_blocks]{"buffer"}.$code_blocks[$#code_blocks]{"comments"};
# do further processing of code_line here
...
# buffer needs more raw_buffer from this point down
          ...
          chompm($code_blocks[$#code_blocks]{"raw_buffer"}); # remove linefeeds etc
          if ($code_blocks[$#code_blocks]{"raw_buffer"} =~ /^([^#]*(&|))(\s*#.*)$/) { # remove comments
...
# if raw_buffer is to be a continuation need to deal with trailing and possibly leading continuation symbols
        if (nonempty($code_blocks[$#code_blocks]{"buffer"}) && $code_blocks[$#code_blocks]{"buffer"} =~ /&\s*$/ ) {
          $code_blocks[$#code_blocks]{"buffer"} = $`; # strip off continuation symbol and trailing spaces from buffer
        ...
        $code_blocks[$#code_blocks]{"buffer"} .= $code_blocks[$#code_blocks]{"raw_buffer"};
        $code_blocks[$#code_blocks]{"raw_buffer"} = '';
      }

# # if code_line, buffer and raw_buffer are empty then we need to fill raw_buffer, or detect end-of-file
#       if (empty($code_blocks[$#code_blocks]{"code_line"}) && empty($code_blocks[$#code_blocks]{"buffer"}) && empty($code_blocks[$#code_blocks]{"raw_buffer"})) {
#         $code_blocks[$#code_blocks]{"raw_buffer"} = <$handle> or last; # exit loop if end of file is reached
# #       ($code_blocks[$#code_blocks]{"raw_buffer"} = <$code_blocks[$#code_blocks]{"handle"}>) or last; # exit loop if end of file is reached
# #       $code_blocks[$#code_blocks]{"raw_buffer"} = <${$code_blocks[$#code_blocks]{handle}}> or last; # exit loop if end of file is reached

# # if code_line and buffer are empty, then fill buffer
#       } elsif (empty($code_blocks[$#code_blocks]{"code_line"}) && empty($code_blocks[$#code_blocks]{"buffer"})) {
#         print UNWRAPPED_INPUT $indent x $#code_blocks,$code_blocks[$#code_blocks]{"raw_buffer"};
#         $code_blocks[$#code_blocks]{"raw_buffer"} = '';
...
sub parse_solver_code_line {
  ...
  my ($line, $type, $name, $cunits, $units, $multiplier, $mvar,
    ...
    $condition, $key, $append, $cancel, $default, $masread, $repeats, $include_type);
